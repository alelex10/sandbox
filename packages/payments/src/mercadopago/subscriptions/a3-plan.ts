...
/**
 * Pure builder for the exact snake_case MP request body `createPlan` sends.
 * Extracted verbatim from `createPlan` so preview endpoints can render the
 * true MP body without calling MP. No I/O, no randomness — deterministic
 * from `input` alone.
 */
export function buildPlanBody(input: CreatePlanInput) {
  return {
    reason: input.reason,
...
/**
 * Pure builder for the exact snake_case MP request body `subscribeToPlan`
 * sends. Extracted verbatim from `subscribeToPlan` so preview endpoints can
 * render the true MP body without calling MP. No I/O, no randomness —
 * deterministic from `input` alone.
 *
...
export function buildSubscribeBody(input: SubscribeToPlanInput): any {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const body: any = {
